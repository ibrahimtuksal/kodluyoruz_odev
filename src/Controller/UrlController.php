    ...
    public function create(Request $request, ValidatorInterface $validator): Response
    {
        ...
        if (count($errorMessages)===0){
            ...
            $url_item->setUrl($url)
                ...
                ->setClickCount($url_item->getClickCount()+1)
                ->setIsPublic(true)
            ...
            if( is_null( $this->getUser() ) ) {
                ...
                $url_item->setUserId($user_id);
                $userurl=$this->getDoctrine()->getManager()->getRepository(User::class)->find($user_id);
                $userurl->setUrlCount($userurl->getUrlCount()+1);
                $em->persist($userurl);
            }
            $em->persist($url_item);
